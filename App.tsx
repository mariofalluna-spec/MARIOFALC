...
const App: React.FC = () => {
  ...
  return (
    <div 
      ref={containerRef}
      className="relative w-full overflow-x-hidden bg-slate-950 text-slate-200 selection:bg-cyan-500/30 font-sans"
    >
      ...
      <div className="relative z-20 flex flex-col w-full">
        ...
        <section className="relative w-full min-h-[100dvh] flex flex-col">
          ...
          {/* Header */}
          <header className="w-full p-6 md:px-12 md:py-8 flex flex-col xl:flex-row items-center justify-between bg-gradient-to-b from-black/90 to-transparent backdrop-blur-[1px] gap-6 xl:gap-0 z-30">
            {/* Logo */}
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
               <span className="text-[10px] md:text-xs font-medium tracking-[0.25em] text-slate-500 uppercase pb-1 md:pb-0">Especialistas en:</span>
               <nav className="flex flex-wrap justify-center gap-2 md:gap-3">
                {specialties.map((spec) => (
                  <div 
                    key={spec.name} 
                    className="group/pill flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-slate-900/40 border border-white/5 backdrop-blur-sm text-[10px] md:text-sm font-medium text-slate-300 transition-all duration-300 ease-out cursor-pointer hover:text-white hover:bg-gradient-to-r hover:from-cyan-500 hover:to-blue-600 hover:border-transparent hover:shadow-[0_0_20px_rgba(6,182,212,0.4)]"
                  >
                    <spec.icon size={12} className="text-slate-400 group-hover/pill:text-white transition-colors md:w-[14px] md:h-[14px]" />
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-4 z-20">
             <h1 className="text-4xl md:text-5xl lg:text-7xl font-serif italic text-center text-white/95 drop-shadow-[0_4px_10px_rgba(0,0,0,1)] leading-tight tracking-wide opacity-90">
               Dale luz a tu sonrisa <br/>
               <span className="text-amber-100/90 not-italic font-light text-2xl md:text-4xl mt-4 md:mt-6 block tracking-[0.25em] uppercase text-shadow-gold">hoy mismo</span>
             </h1>
          </div>
        ...
        <footer className="w-full bg-gradient-to-t from-black via-slate-950 to-transparent py-16 md:py-24 px-6 md:px-16 border-none">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12 text-center md:text-left">
